...
def criteria2_do_one_line(context_before_include_anchor, anchor_sentence, question_text, context, client, model_name):
    prompt_find_answer = "article: " + ''.join(context_before_include_anchor) + "\n" + "After reading sentence:'" + anchor_sentence + "', a reader asked the following question.\n" + "Q:" + question_text + "A:"
    generated_answer = get_chatgpt_response(prompt_find_answer, client, model_name)
    prompt_find_closest_sent = "article:" + ''.join(context) + "\n" + "Which sentence in the article is closest to the sentence: '"  + generated_answer + "'\n" "A:"
    response = get_chatgpt_response(prompt_find_closest_sent, client, model_name)
    filtered_response = filter_response([response], context)
    return filtered_response, response, (prompt_find_answer, prompt_find_closest_sent)
