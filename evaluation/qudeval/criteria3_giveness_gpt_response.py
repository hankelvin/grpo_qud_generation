...
def criteria3_do_one_line(context_before_include_anchor, question_text, answer_sentence, client, model_name):
    response = []
    prompt = "Context: \n" + '\n'.join(context_before_include_anchor) + "\n\n" + "Question: \n" + question_text +"\n"+ "Answer: \n" + answer_sentence + "\n"
    r = get_chatgpt_response(prompt, client, model_name)
    response.append(r)
    filtered_response = filter_response(pd.Series(response))
    assert len(response) == 1, len(response)
    return filtered_response.tolist(), r, prompt
