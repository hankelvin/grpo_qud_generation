...
class Query:
    ...
    ### CHANGE START ###
    def asdict(self):
        return {'text': self.text, 'qid': self.qid, 'context': self.context,
                'anchor': self.anchor, 'answer': self.answer}
    ### CHANGE END ###
...
class Candidate:
    ...
    ### CHANGE START ###
    def asdict(self):
        return {'docid': self.docid, 'score': self.score, 'doc': self.doc,
...
class Request:
    ...
    ### CHANGE START ###
    def asdict(self):
        return {'query': self.query.asdict(), 'candidates': [c.asdict() for c in self.candidates]}
    ### CHANGE END ###
...
class Result:
    ...
    # prompt_repr:    list[tuple] = field(default_factory=list)
    # num_hops:       int         = None
