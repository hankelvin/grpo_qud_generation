...
from tools.rank_llm.src.rank_llm.data import RankingExecInfo, Request, Result
### CHANGE END ###
...
class RankLLM(ABC):
    ...
    def permutation_pipeline(
        ...
        ### CHANGE START ###
        prompt: str = None, # passing an already produced prompt. e.g. for RM SFT
        ### CHANGE END ### 
    ) -> Result:
        """
        ...
        """
        ### CHANGE START ###
        if (prompt is None and not self.use_openai_client) or self.use_openai_client:
            prompt, in_token_count, messages = self.create_prompt(result, rank_start, rank_end, 
                                                    qud_criteria = qud_criteria,)
        else: 
            in_token_count = self.get_num_tokens(prompt)
            messages       = [] 
        ...
        if self.use_openai_client: 
            raw_outputs = None
            outputs, out_token_count, cot_output = \
                self.run_openai_client(current_window_size = current_window_size, 
                                       messages = messages)
            prompt = '\n'.join([f'{m["role"]}: {m["content"]}' for m in messages])
            ### CHANGE END ###
        else: 
            ### CHANGE START ####
            outputs, out_token_count, cot_output, raw_outputs = self.run_llm(
                prompt = prompt, current_window_size = current_window_size)
            
        if getattr(self.cfg.ranker_args, 'use_past_key_values', False):
            prompt = self.pkv_prompt + prompt
        ...
        ranking_exec_info = RankingExecInfo(
            prompt, permutation, in_token_count, out_token_count, raw_outputs, cot_output, 
        )
    ...
    def sliding_windows(
        ...
        while end_pos > rank_start and start_pos + step != rank_start:
            ...
            rerank_result = self.permutation_pipeline(rerank_result, start_pos, end_pos, logging, qud_criteria, prompt)            
